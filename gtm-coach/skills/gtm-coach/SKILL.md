...
categories — `~~meeting recording` (required), and `~~calendar` / `~~email` / `~~crm` if
present (see `CONNECTORS.md`). If no `~~meeting recording` tool is connected, stop and tell the
...
- What they can do next: backfill older history (`sync-memory`), prep for an upcoming call
  ...
  mine patterns (`gtm-patterns`), or score calls/reps (`coaching-scorecard`).
